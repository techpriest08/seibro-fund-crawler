...
- [x] **PyInstaller exe 패키징 (프로토타입)** — `gui_app.py` (Tkinter, 펀드명
      ...
      확인함(창 핸들 생성 확인). **한계**: Chromium 브라우저 바이너리는 exe에 번들
      안 됨 — 실행 컴퓨터에 `playwright install chromium` 이 미리 되어 있어야 함.
      완전 독립형(다른 PC에 exe만 복사해서 실행)으로 만들려면 Chromium 폴더까지
      같이 묶는 작업 추가 필요 (다음 과제)
