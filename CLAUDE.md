...
### 다음 세션 TODO (2026-07-07 확정, 다른 컴퓨터에서 이어서 작업 예정)
- [ ] **분배율 로그가 안 보이는 문제 확인** — `summarize_distribution_yield()`는
      이미 구현돼 있는데 사용자가 실행 결과에서 못 봤다고 함. Windows 콘솔
      cp949 인코딩과 UTF-8 로그 문자열이 안 맞아서 한글이 깨져 보였을
      가능성이 큼(실제로 `1�Ⱓ �й� ...` 식으로 깨진 출력을 확인한 적 있음).
      `chcp 65001` 또는 logging 핸들러 인코딩 명시로 먼저 재현/해결 시도
- [ ] **펀드 총자산(AUM) 변화 추적 기능** — 분배 시점 전후로 펀드 순자산이
      얼마나 변했는지, 이번 분배금이 자산의 몇 %인지 계산. AUM 데이터를
      세이브로 어디서 가져올지 아직 못 찾음 — 펀드종합정보 메뉴나 KOFIA
      쪽에 있는지부터 확인 필요
